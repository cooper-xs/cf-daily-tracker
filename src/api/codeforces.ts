...
/**
 * 获取单个用户在指定日期范围内的所有提交记录（自动分页）
 * @param handle 用户名
 * @param startSeconds 开始时间戳（秒）
 * @param endSeconds 结束时间戳（秒）
 * @returns 提交记录列表
 */
async function getUserSubmissionsByDateRange(
  handle: string,
  startSeconds: number,
  endSeconds: number
): Promise<CFSubmission[]> {
  ...
  while (hasMore) {
    const submissions = await getUserStatus(handle, from, batchSize);
    ...
    // 检查是否还需要继续获取：
    // 如果这批记录的最早一条仍在日期范围之后，继续获取下一批
    const earliestSubmission = submissions[submissions.length - 1];
    const shouldContinue = earliestSubmission.creationTimeSeconds > startSeconds;

    if (!shouldContinue || submissions.length < batchSize) {
      hasMore = false;
    } else {
      from += batchSize;
    }
...
export async function getUsersSubmissionsByDateRange(
  ...
  for (const handle of handles) {
    try {
      const submissions = await getUserSubmissionsByDateRange(handle, startSeconds, endSeconds);
      result.set(handle, submissions);
    } catch (error) {
      console.error(`获取用户 ${handle} 的提交记录失败:`, error);
      result.set(handle, []);
    }
